...
async fn parse_openai_sse(response: reqwest::Response, on_chunk: &mut impl FnMut(&str)) -> Result<String, String> {
    ...
    if let Some(fallback) = extract_openai_json_text(&raw_response)? {
        on_chunk(&fallback);
        return Ok(fallback);
    }

    Err("模型返回了空内容：未收到任何 SSE 文本块，也未能从普通 JSON 响应中解析出 choices[0].message.content。请在管理员设置里测试模型连接，并确认该端点支持 OpenAI-compatible chat/completions。".into())
}
...
fn provider_json_error(error: &serde_json::Value) -> String {
    ...
    serde_json::to_string(error).unwrap_or_else(|_| "模型服务返回错误".into())
}

fn extract_openai_json_text(raw_response: &str) -> Result<Option<String>, String> {
    ...
    // Some proxies prepend status/debug lines before the final JSON. Try the last JSON object.
