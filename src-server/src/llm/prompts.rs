...
pub fn build_prompt(context_type: &str, params: &Value) -> (String, String) {
    match context_type {
        ...
        "longform_idea_incubator" => build_longform_idea_incubator_prompt(params),
        _ => (
...
fn build_user_message(
    ...
    if let Some(c) = concept {
     ...
     // Include user's imported source material (full script/novel/story)
     if let Some(imported) = init["importedScript"]
         .as_str()
         .or_else(|| init["imported_script"].as_str())
         .filter(|s| !s.trim().is_empty())
     {
         msg.push_str("## 用户提供的原始素材\n\n");
         msg.push_str(imported);
         msg.push_str("\n\n---\n\n");
     }
...
fn build_longform_idea_incubator_prompt(params: &Value) -> (String, String) {
    let system = r#"你是 ScriptStack 的「长故事孵化器」。

你的任务：把用户的一句话想法或短概念，扩展成可长期写作、可拆分成单集/单章，并且能交给后续 23 个原始提示词正本流程继续创作的中文项目资料。

硬规则：
1. 只输出 JSON 对象，不要 markdown 围栏，不要解释。
2. 输出必须是中文；专有名词可保留英文。
3. 不要把想法直接写成完整剧本。你要产出故事圣经、长线结构和分集/分章可写单元。
4. 每个 episodeOutlines[n].sourceForWorkflow 必须是一段可直接作为八步工作流输入的中文资料包，包含本集故事目标、承接信息、人物状态、关键场景、关键道具、开场钩子、结尾钩子。
5. 角色、世界观、道具、场景必须能为后续资产矩阵提供高质量源头，但不要在这里生成英文视觉提示词。
6. 如果用户只给一句想法，你必须主动补足类型承诺、主角欲望、核心冲突、反派/阻力系统、长期悬念和每集推进逻辑。
7. 分集不能互相重复；每一集必须有新的因果推进和结尾钩子。

输出 JSON schema：
{
  "seriesTitle": "项目标题",
  "format": "短剧/网剧/电影长片/小说/单元剧",
  "corePremise": "核心概念扩写",
  "logline": "一句话故事钩子",
  "genrePromise": ["类型承诺"],
  "storyBible": {
    ...
    "sceneSeeds": ["可复用场景"],
    "propSeeds": ["关键道具"],
    "rulesAndConstraints": ["世界规则或创作约束"]
  },
  "seasonOutline": {
    "totalEpisodes": 12,
    "structure": "整体结构说明",
    "acts": [
      { "name": "阶段名", "episodeRange": "1-4", "function": "阶段功能" }
    ]
  },
  "episodeOutlines": [
    {
      ...
      "openingHook": "开场钩子",
      "mainConflict": "本集主冲突",
      "keyBeats": ["节拍1", "节拍2", "节拍3"],
      "characterShift": "人物变化",
      "continuityNotes": "承接/伏笔/不能忘记的信息",
      "endingHook": "结尾钩子",
      "sourceForWorkflow": "给八步工作流使用的本集中文资料包"
    }
  ],
  "quality": {
    ...
    let user = format!(
        "## 用户原始想法\n{}\n\n## 目标形态\n{}\n\n## 计划集数/章节数\n{}\n\n## 单集/单章目标时长\n{}\n\n## 题材/类型\n{}\n\n## 受众\n{}\n\n## 语气与风格\n{}\n\n## 用户补充\n{}\n\n请按 schema 输出 JSON 对象。",
        params["idea"].as_str().unwrap_or(""),
        params["format"].as_str().unwrap_or("short_drama"),
        params["episodeCount"].as_u64().unwrap_or(12),
        params["episodeDuration"].as_str().unwrap_or("2分钟"),
        params["genres"].as_str().unwrap_or(""),
        params["audience"].as_str().unwrap_or("短视频观众"),
        params["tone"].as_str().unwrap_or("强冲突、强钩子、强情绪"),
        params["notes"].as_str().unwrap_or("")
    );
