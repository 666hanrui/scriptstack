...
fn build_assets_json(conn: &Connection, task_id: &str) -> Result<String, String> {
    crate::services::visual_prompt_service::build_visual_standard_context(conn, task_id)
}
...
fn json_string_field(raw: &serde_json::Value, keys: &[&str], default: &str) -> String {
    keys.iter()
...
fn json_string_array_field(raw: &serde_json::Value, keys: &[&str]) -> Vec<String> {
    keys.iter()
...
fn normalize_v2_shot(raw: &serde_json::Value, scene_index: usize) -> V2Shot {
    V2Shot {
        ...
        scene_index,
        shot_type: json_string_field(raw, &["shotType", "shot_type"], ""),
        title_bar: json_string_field(raw, &["titleBar", "title_bar"], ""),
        mount: json_string_field(raw, &["mount"], ""),
        camera: json_string_field(raw, &["camera"], ""),
        opening_frame: json_string_field(raw, &["openingFrame", "opening_frame"], ""),
        closing_frame: json_string_field(raw, &["closingFrame", "closing_frame"], ""),
        connection: if scene_index == 0 {
            String::new()
        } else {
            json_string_field(raw, &["connection"], "")
        },
        transition: json_string_field(raw, &["transition"], ""),
        dual_anchor: json_string_field(raw, &["dualAnchor", "dual_anchor"], ""),
        main_prompt: json_string_field(raw, &["mainPrompt", "main_prompt"], ""),
        compulsory_declaration: json_string_field(
            raw,
            &["compulsoryDeclaration", "compulsory_declaration"],
            COMPULSORY_DECLARATION_DEFAULT,
        ),
        must_show: json_string_field(raw, &["mustShow", "must_show"], ""),
        quality_route: json_string_field(raw, &["qualityRoute", "quality_route"], ""),
        imaging_style: json_string_field(raw, &["imagingStyle", "imaging_style"], ""),
        quality_baseline: json_string_field(
            raw,
            &["qualityBaseline", "quality_baseline"],
            QUALITY_BASELINE_DEFAULT,
        ),
        reference: json_string_field(raw, &["reference"], ""),
        micro_expressions: json_string_field(raw, &["microExpressions", "micro_expressions"], ""),
        nail_lines: json_string_field(raw, &["nailLines", "nail_lines"], ""),
        e15: json_string_field(raw, &["e15"], ""),
        asset_refs: json_string_array_field(raw, &["assetRefs", "asset_refs"]),
    }
}
...
fn scene_index_from_value(value: &serde_json::Value, fallback: usize) -> usize {
    value
        .get("sceneIndex")
        .or_else(|| value.get("scene_index"))
...
fn load_existing_seedance_values(conn: &Connection, task_id: &str) -> Vec<serde_json::Value> {
...
pub async fn run_prompt_group_generation(
    ...
    let mut previous_context: Option<serde_json::Value> = None;
    if scene_index > 0 {
        let all_existing = load_existing_seedance_values(conn, task_id);
        if let Some(prev_group) = all_existing
            .iter()
            .enumerate()
            .find_map(|(fallback, g)| {
                (scene_index_from_value(g, fallback) == scene_index - 1).then_some(g)
            })
        {
    ...
    let existing_values = load_existing_seedance_values(conn, task_id);
    let mut all_seedance: Vec<V2Shot> = existing_values
        .iter()
        .enumerate()
        .map(|(fallback, value)| normalize_v2_shot(value, scene_index_from_value(value, fallback)))
        .collect();
    all_seedance.retain(|g| g.scene_index != scene_index);
    all_seedance.push(group.clone());
    all_seedance.sort_by_key(|g| g.scene_index);
    for g in all_seedance.iter_mut() {
        g.shot_number = g.scene_index + 1;
    }
