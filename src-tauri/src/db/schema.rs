...
fn ensure_visual_prompt_output_columns(conn: &Connection) -> Result<(), Box<dyn std::error::Error>> {
    // Existing local databases may have been created before Visual Prompt Forge phase 5.
    // SQLite has no "ADD COLUMN IF NOT EXISTS", so duplicate-column errors are ignored.
    let migrations = [
